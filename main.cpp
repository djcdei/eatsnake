...
public:
    ...
    virtual void draw()
    {
        try // 限制游戏区域
        {
            if (x < 50 || y < 50 || x >= 550 || y >= 450 || CollisionBodyFlag == 1) // 判断节点的坐标是否超出范围
            {
                /*抛出碰壁错误*/
...
public:
    ...
    void Change()
    {
        // 生成 x 坐标，确保是 10 的整数倍，并且在 (50, 540) 之间
        int minX = 50;
        int maxX = 540;
        int xRange = (maxX - minX + 1) / 10;
        x = minX + (rand() % xRange) * 10;

        // 生成 y 坐标，确保是 10 的整数倍，并且在 (50, 440) 之间
        int minY = 50;
        int maxY = 440;
        int yRange = (maxY - minY + 1) / 10;
        y = minY + (rand() % yRange) * 10;
...
public:
    Snake(int x = 0, int y = 0) : Sprite(x, y) // 使用默认参数，当没有参数传进来的时候就是默认构造函数
    {
        ...
        if (ResumeGameFlag == 0)
        {
            ...
            // 初始化3节蛇
            nodes.push_back(Sprite(70, 50, 0xff0000));
            nodes.push_back(Sprite(60, 50, 0x00ff00));
    ...
    void draw() // 把蛇所有的节点绘制出来
    {
        ...
        if (!nodes.empty())
        {
            ...
            if ((dir == 71 || dir == 67 || dir == 49 || dir == 52) && StopGameFlag == 1) // 限制在运动的时候才清空最后一个节点
            {
                ...
                lcd.lcd_fill(lastNode.getX(), lastNode.getY(), 10, 10, 0xffffff);
            }
        }
        else
        {
...
public:
    ...
    void run()
    {
        KeyPressed key(gamescene.getSnake()); // 键盘监听线程
        while (true)
        {
