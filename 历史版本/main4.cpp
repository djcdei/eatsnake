...
public:
    ...
    void ClearLastNode()
    {
        ...
        if ((dir == 71 || dir == 67 || dir == 49 || dir == 52) && StopGameFlag == 1) // 限制在运动的时候才清空最后一个节点
        {
            ...
            if ((lastNode.getX() > 50) || (lastNode.getX() < 500) || (lastNode.getY() > 50) || (lastNode.getY() < 400))
            {
                if (dir == 49 || dir == 52)
                    lcd.lcd_fill(lastNode.getX() - 35, lastNode.getY(), 35, 35, 0xffffff);
                else if (dir == 67 || dir == 71)
                    lcd.lcd_fill(lastNode.getX(), lastNode.getY() - 35, 35, 35, 0xffffff);
            }
        }
    }
...
public:
    ...
    void run()
    {
        std::cout << "-----------------------游戏场景的run--------------------" << std::endl;
        // std::cout << "移动前x:" << snake.getX() << "y:" << snake.getY() << std::endl;
        food.draw();      // 绘制食物
        snake.draw();     // 绘制蛇身
        snake.BodyMove(); // 移动蛇
                          // sleep(3);
        usleep(1000 * 300);
        // std::cout << "移动后x:" << snake.getX() << "y:" << snake.getY() << std::endl;
        SnakeEatFood(); // 吃食物
...
public:
    ...
    void StartFun()
    {
        while (!shouldExit)
        {
            ...
            std::cout << "当前方向" << gamescene.getSnake().dir << std::endl;
        }
