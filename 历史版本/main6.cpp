...
public:
    ...
    void ParseJson(const std::string &filepath)
    {
        ...
        // // 获取"food"
        // cJSON *tempfood = cJSON_GetObjectItem(root, "food");
        // if (tempfood->type == cJSON_Array)
        // {
        //     cJSON *x = cJSON_GetObjectItem(tempfood, "x");
        //     cJSON *y = cJSON_GetObjectItem(tempfood, "y");
        //     cJSON *color = cJSON_GetObjectItem(tempfood, "color");
        //     if (x->type == cJSON_Number && y->type == cJSON_Number)
        //     {
...
public:
    ...
    void run()
    {
        ...
        if (StopGameFlag == 1)
        {
            ...
            showBackgroundColor(50, 0, 32); // 恢复背景颜色
    ...
    void createJson()
    {
        ...
        try
        {
            ...
            // 先写入积分和方向，这里要注意写入的是数字，而不是字符串，方便之后解析
            outfile << "{" << std::endl;
    ...
    void run()
    {
        while (true)
        {
            ...
            if (ts_sta == 0)
            {
                ...
                else if (170 < ts_x && ts_x < 325 && 195 < ts_y && ts_y < 235 && OverGameFlag == 1) // 重新开始
                {
