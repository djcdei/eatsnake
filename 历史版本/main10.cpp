...
public:
    Sprite() {}
    Sprite(int _x, int _y, int _color = 0x00ff00, std::string _name="") : x(_x), y(_y), color(_color), name(_name) {}
    ...
    virtual void draw()
    {
        try // 限制游戏区域
        {
            if (x < 50 || y < 50 || x >= 530 || y >= 430 || CollisionBodyFlag == 1) // 判断节点的坐标是否超出范围
            {
                /*抛出碰壁错误*/
                // music.switchMusic("/djc/snake/bgm/bgm2.mp3");
                music.switchMusic("/djc/snake/bgm/audio/over1.mp3");
                sleep(1.5);
                throw "碰壁！！游戏结束";
            }
        }
...
protected:
    ...
    int y;
    int color;        // 节点颜色
    std::string name; // 节点名字，可选择不使用色块，使用显示bmp图片的时候就需要读取图片名字
};
...
public:
    Food() : Sprite(0, 0)
    {
        ...
        scoreToname[10] = "/djc/snake/food/food2.bmp";
        scoreToname[20] = "/djc/snake/food/food3.bmp";
        scoreToname[30] = "/djc/snake/food/food4.bmp";
    ...
    void draw() // 绘制食物
    {
        ...
        {
            if (scoreToname.find(score) != scoreToname.end()) // 在map容器里查找该积分对应的图片名字
            {
                this->name = scoreToname[score];
                lcd.showBmp(name.c_str(), x, y);
            }
        }
    ...
    void Change()
    {
        // 生成 x 坐标，确保是 20 的整数倍，并且在 (50, 530) 之间
        x = 50 + (rand() % 25) * 20;
        // 生成 y 坐标，确保是 20 的整数倍，并且在 (50, 430) 之间
        y = 50 + (rand() % 20) * 20;
...
private:
    ...
    std::map<int, std::string> scoreToname;   // 存放积分跟图片名字的映射表，根据积分选择不同的bmp图片显示
    int score;
};
...
public:
    Snake(int x = 0, int y = 0) : Sprite(x, y) // 使用默认参数，当没有参数传进来的时候就是默认构造函数
    {
        ...
        if (ResumeGameFlag == 0)
        {
            ...
            // 初始化3节蛇
            nodes.push_back(Sprite(80, 50, 0xff0000,"snake.bmp"));
            nodes.push_back(Sprite(65, 50, 0x00ff00,"body.bmp"));
            nodes.push_back(Sprite(50, 50, 0x0000ff,"body.bmp"));
            this->dir = 0;      // 刚进入游戏蛇是停止状态
            this->tempdir = 52; // 点击开始运动按钮，蛇默认向右移动
            this->integral = 0; // 初始积分为0
        }
    ...
    // 蛇身增加
    void AddBody(int _x, int _y, int _color)
    {
        /*添加一个容器，存储若干个int类型颜色值，再通过随机数选出来，添加进蛇身容器，
        还可以根据不同颜色类型，添加不同等级的得分，（因为每次添加蛇身说明）*/
        nodes.push_back(Sprite(_x, _y, _color));
    }
    ...
    void draw() // 把蛇所有的节点绘制出来
    {
        ...
        if (!nodes.empty())
        {
            ...
            if ((dir == 71 || dir == 67 || dir == 49 || dir == 52) && StopGameFlag == 1) // 限制在运动的时候才清空最后一个节点
            {
                ...
                lcd.lcd_fill(lastNode.getX(), lastNode.getY(), 20, 20, 0xffffff);
            }
        }
        else
        {
...
public:
    ...
    void SnakeEatFood()
    {
        if (snake.Collision(food))
        {
            ...
            music.switchMusic("/djc/snake/bgm/audio/over2.mp3");

            usleep(1000 * 200);
            snake.setSnakeScore(food.getFoodScore()); // 这里要先把当前食物的积分加上去，不能放在后面，加完之后食物被刷新了积分也会刷新·
            std::cout << "吃到积分为:" << food.getFoodScore() << std::endl;
            std::cout << "吃到颜色为:" << food.getNodeColor() << std::endl;
            snake.AddBody(food.getX(), food.getY(), food.getNodeColor()); // 添加蛇身
            food.Change();                                                // 改变食物位置
            CollisionFoodFlag = 1;
        }
        else if (CollisionFoodFlag == 1 && !snake.Collision(food))
        {
            music.switchMusic("/djc/snake/bgm/bgm1.mp3");
            CollisionFoodFlag = 0;
        }
    }
    // 判断碰撞（是否吃到自己身体）
    void SnakeEatSelf()
    {
        if (snake.Collision(snake))
        {
            ...
            CollisionBodyFlag = 1; // 全局变量，标志吃到自己的身体
        }
        else
        {
            CollisionBodyFlag = 0;
...
public:
    ...
    void StartFun()
    {
        ...
        // 限制
        if (OverGameFlag == 0 && StopGameFlag == 0) // 不是从游戏结算界面退出的，保证游戏存档的合理性
        {
        ...
        while (true)
        {
            ...
            if (ts_sta == 0)
            {
                ...
                else if (0 < ts_x && ts_x < 100 && 0 < ts_y && ts_y < 100 && StartGameFlag == 1 && OverGameFlag == 0) // 退出游戏界面，回到开始界面
                {
                    ...
                    music.playMusic("/djc/snake/bgm/bgm2.mp3");
                    StartGameFlag = 0;
                ...
                else if (240 < ts_x && ts_x < 565 && 345 < ts_y && ts_y < 397 && OverGameFlag == 1) // 结算界面返回游戏
                {
                    ...
                    OverGameFlag = 0;               // 游戏线程退出后再赋值为0，因为如果碰墙后再结束游戏，不能进行存档
                }

                else if (240 < ts_x && ts_x < 565 && 275 < ts_y && ts_y < 325 && OverGameFlag == 1) // 结算界面重新开始
                {
                    std::cout << "结算界面点击重新开始" << std::endl;
                    music.playMusic("/djc/snake/bgm/bgm1.mp3");
